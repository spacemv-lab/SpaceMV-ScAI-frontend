...
export abstract class KeepTrackPlugin {
  ...
  addHtml(): void {
    ...
    if (this.bottomIconLabel) {
      ...
      this.bottomIconElementName = this.bottomIconElementName ?? `${bottomIconSlug}-bottom-icon-${this.id}`;
    }
  ...
  private generateSideMenuHtml_() {
    ...
    const downloadIconHtml = this.downloadIconCb
      ? keepTrackApi.html`
  ...
  showBottomIcon(): void {
    ...
    if (bottomIconElement) {
      if (bottomIconElement.getAttribute('data-initially-hidden') === 'true') {
  ...
  addContextMenuLevel1Item(html: string): void {
    keepTrackApi.on(KeepTrackApiEvents.rightBtnMenuAdd, () => {
  ...
  addBottomIcon(icon: Module, isDisabled = false): void {
    keepTrackApi.on(KeepTrackApiEvents.uiManagerInit, () => {
      ...
      if (!showButtonArr.includes(this.id)) {
        // 添加标记属性，表明这是需要隐藏的按钮
        button.setAttribute('data-initially-hidden', 'true');
      }
   ...
   */
  registerBottomMenuClicked(
    callback: () => void = () => {
      // Do Nothing
    },
  ) {
    if (this.isRequireSensorSelected && this.isRequireSatelliteSelected) {
      keepTrackApi.on(KeepTrackApiEvents.selectSatData, (obj: BaseObject): void => {
    ...
    if (this.isRequireSensorSelected && !this.isRequireSatelliteSelected) {
      keepTrackApi.on(KeepTrackApiEvents.setSensor, (sensor, sensorId): void => {
        if (!sensor && !sensorId) {
          this.setBottomIconToDisabled();
          this.setBottomIconToUnselected();
        } else {
    ...
    keepTrackApi.on(KeepTrackApiEvents.bottomMenuClick, (iconName: string): void => {
      if (iconName !== 'menu-constellations') {
        keepTrackApi.getUiManager().initAllLayersToOff();
      }
  ...
  registerHelp(helpTitle: string, helpText: string) {
    keepTrackApi.on(KeepTrackApiEvents.onHelpMenuClick, (): boolean => {
      if (this.isMenuButtonActive) {
  ...
  registerHideSideMenu(bottomIconElementName: string, slideCb: () => void): void {
    keepTrackApi.on(KeepTrackApiEvents.hideSideMenus, (): void => {
      slideCb();
