...
import { SatInfoBoxSensorLoad } from './sat-info-box-sensorload/sat-info-box-sensorload';
...
import { SatellitePlugin } from './satellite/satellite';
import { ImportConstellationPlugin } from './import-constellation/import-constellation';
import { LoadSetPlugin } from './load-set/load-set';
import { GroundTargetPlugin } from './ground-target/ground-target';
import { ConstellationDeclineSimulationPlugin } from './constellation-decline-simulation/constellation-decline-simulation';
import { AlgorithmModelPlugin } from './algorithm-model/algorithm-model';
...
export const loadPlugins = async (keepTrackApi: KeepTrackApi, plugins: KeepTrackPluginsConfiguration): Promise<void> => {
  ...
  try {
    const pluginList: { init: () => void | Promise<void>; config?: { enabled: boolean } }[] = [
      ...
      { init: () => new SatInfoBoxSensorLoad().init(), config: plugins.SatInfoBoxSensorLoad},
      ...
      { init: () => new SatellitePlugin().init(), config: plugins.SatellitePlugin },
      { init: () => new ImportConstellationPlugin().init(), config: plugins.ImportConstellationPlugin },
      { init: () => new LoadSetPlugin().init(), config: plugins.LoadSetPlugin },
